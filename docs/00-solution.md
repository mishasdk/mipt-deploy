...
Все сервисы доступны через nginx-шлюз (порт 80) на `http://111.88.248.10` (до времени защиты).
Доступ под auth gateway, кроме Grafana и Airflow (своя авторизация).

| Сервис | Endpoint | Назначение |
|---|---|---|
| ML-сервис | http://111.88.248.10/ | API инференса |
| ML-сервис Swagger | http://111.88.248.10/docs | проверка API  |
| MLflow | http://111.88.248.10/mlflow | трекинг экспериментов и registry |
| Airflow | http://111.88.248.10/airflow | оркестрация обучения |
| Prometheus | http://111.88.248.10/prometheus | сбор метрик |
| Grafana | http://111.88.248.10/grafana | дашборды и алерты |
